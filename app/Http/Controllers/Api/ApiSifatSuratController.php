...
class ApiSifatSuratController extends Controller
{
    public function list(Request $request)
    {
        $model = SifatSurat::where('is_delete', 0)->get();
        ...
        return DataTables::of($model)
            ...
            ->addColumn('aksi', function ($data) {
                $button = '';
                $button .= '<a href="' . url("setting/sifat-surat/update/" . $data->id) . '" title = "Edit" data-id="' . $data->id . '" class="btn btn-warning btn-sm"> <i class="fas fa-edit"></i> Edit</a>';
                $button .= '&nbsp';
                $button .= '&nbsp';
