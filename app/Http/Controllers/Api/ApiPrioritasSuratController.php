...
class ApiPrioritasSuratController extends Controller
{
    public function list(Request $request)
    {
        $model = PrioritasSurat::where('is_delete', 0)->get();
        ...
        return DataTables::of($model)
            ...
            ->addColumn('aksi', function ($data) {
                $button = '';
                $button .= '<a href="' . url("setting/prioritas-surat/update/" . $data->id) . '" title = "Edit" data-id="' . $data->id . '" class="btn btn-warning btn-sm"> <i class="fas fa-edit"></i> Edit</a>';
                $button .= '&nbsp';
                $button .= '&nbsp';
